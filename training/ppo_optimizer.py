...
if __name__ == "__main__":
    """Unit test — python ppo_optimizer.py"""
    import sys
    ...
    BS = config['training']['buffer_size']

    print("=" * 60)
    print("PPOOptimizer Unit Test")
    print("=" * 60)

    # Fill a rollout buffer
    buf = RolloutBuffer(config, device)
    for t in range(BS):
        lo  = torch.randn(N, config['observation']['local_obs_dim'])
        ns  = torch.randn(N, K, 6)
        nm  = torch.zeros(N, K, dtype=torch.bool)
        gs  = torch.randn(N, 17)
        act = torch.randn(N, 4)
        lp  = torch.randn(N)
        rew = torch.rand(N)
        don = torch.zeros(N)
        val = torch.randn(N)
        buf.store(lo, ns, nm, gs, act, lp, rew, don, val)

    buf.compute_gae(torch.randn(N))

    # Record initial weights to verify they change
    ...
    actor_changed  = any(
        ...
        for p, ip in zip(actor.parameters(), initial_actor_params)
    )
    critic_changed = any(
        not torch.allclose(p, ip)
        for p, ip in zip(critic.parameters(), initial_critic_params)
    )

    print(f"\nActor weights changed  : {actor_changed}   ✓")
    print(f"Critic weights changed : {critic_changed}  ✓")
    assert actor_changed,  "Actor weights did not update — check loss computation"
    assert critic_changed, "Critic weights did not update — check loss computation"
